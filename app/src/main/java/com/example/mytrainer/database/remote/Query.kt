...
object Query {
    ...
    fun addUser(user: User) {
        Firestore.create(SQLContract.getRemoteTableName(user), user) { ok, info ->
    ...
    fun addExercise(exercise: Exercise) {
        Firestore.create(SQLContract.getRemoteTableName(exercise), exercise) { ok, info ->
    ...
    fun getAllExercises(callback: (List<Exercise>) -> Unit) {
        return Firestore.getAll(SQLContract.getRemoteTableName(Exercise()), callback)
    }

    fun addTrainingSchedule(schedule: TrainingSchedule) {
        Firestore.create(SQLContract.getRemoteTableName(schedule), schedule) {ok, info ->
            if (ok) {
